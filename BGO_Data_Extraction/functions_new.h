...
inline void pack_dataMain(unsigned char* buf, uint8_t id, uint8_t fe, uint32_t trig,  uint8_t mode, uint8_t N_modules){
...
inline void pack_data(unsigned char* buf, uint8_t fib, uint32_t tim, /*uint32_t chg,*/ uint16_t nbsample){

  packi8((unsigned char*)(buf), fib);
  packi32((unsigned char*)(buf+1), tim);
  //convert32to24((unsigned char*)(buf+5), chg);
...
inline void unpack_data(unsigned char* buf, hit_block_receive &data){
  ...
  //data.hit_time_rec = unpacki24((unsigned char *)(buf+5));
